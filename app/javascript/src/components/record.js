import RecordControls from "src/components/record-controls";
...
const Record = {
  ...
  renderBase: function ({ x, y, width, height, borderSize, fullWidth, fullHeight, color, selected }) {
    ...
    this.draw(this.ctx.fillRect, xCorner + borderSize, yCorner + borderSize, width, height);
  },
  renderText: function ({ x, y, width, height, fullHeight, caption, type, border, borderSize, isZooming }) {
    if (!border) {
      return;
    }
    if (type === "collection") {
    ...
    if (!isZooming) {
      ...
      this.draw(this.ctx.fillRect, xpos - textWidth / 2, ypos - fontHeight, textWidth, fontHeight);
    }
  },
  loadImage: function (props) {
    const cachedImage = imgCache[props.type + props.id];
    if (cachedImage) {
      if (cachedImage.width !== props.width || cachedImage.height !== props.height) {
  ...
  renderImage: function (img, props) {
    ...
    const visibleOpacity = this.getVisibleOpacity(img.width, props.scale, props.canvasScale);
    ...
    if (visibleOpacity > 0 && !props.isZooming) {
      ...
      const yCorner = props.y - props.height / 2 - (BOTTOM_BORDER_FACTOR * props.borderSize) / 2;
  ...
  isPointInRecord: function (props) {
    ...
    this.ctx.save();
    this.transformToItem(fullProps);
    const output = RecordControls.getTypeAtPoint(props.inputX, props.inputY, fullProps);
    this.ctx.restore();
    return output;
  },
  computeTransformedDimensions: function (item, canvas) {
    const { fullWidth, fullHeight } = this.computeBorderSize(item);
    const xMin = item.x - 0.5 * fullWidth * item.scale + canvas.x / canvas.scale;
    const yMin = item.y - 0.5 * fullHeight * item.scale + canvas.y / canvas.scale;
  ...
  isRecordEnclosed: function (item, canvas, x, y, w, h) {
    const { xMin, yMin, xMax, yMax } = this.computeTransformedDimensions(item, canvas);
